"""
process_csr_lt_month_express.py
===============================
...
For each input file, outputs ONE CSV named:
    Tik klientu mokejimai pagal LT laiko zona.csv
(or "Tik klientu mokejimai pagal LT laiko zona (2).csv", etc. if needed)
"""
...
_CONFIG_PATH = Path(__file__).resolve().parent / ".customer_payments_prefs.json"
...
class Cleaner:
    """Convert CSR times to LT time, filter by description + LT month, and sort."""
...
def run_one(src_path: Path, month: int, dest_dir: Path) -> list[Path]:
    ...
    if cleaned.empty:
        # Nothing matching filters in that LT month; no output file
        return []

    # Always use the same base filename
    base_name = "Tik klientu mokejimai pagal LT laiko zona.csv"
